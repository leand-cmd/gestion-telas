...
for folder_path in sorted(images_base.iterdir()):
    ...
    for i in range(num_products):
        ...
        try:
            prod_data = {
                "cod_producto": code,
                "nombre_tejido": f"{collection_name} #{i+1}",
                "categoria": "Tejido",
            ...
            if prod_response.status_code in [200, 201]:
                ...
                print(f"    ✓ {code} (ID: {product_id})")
                created_products += 1
                time.sleep(0.5)  # Pequeño delay entre creaciones
            else:
                print(f"    ✗ {code}: {prod_response.status_code}")
                errors += 1
                ...
                while retry < max_retries and not success:
                    try:
                        ...
                        if retry < max_retries:
                            print(f"    ⟳ Reintentando {retry}/{max_retries}... ({error_msg})")
                            time.sleep(2)  # Esperar 2 segundos antes de reintentar
                        else:
                            print(f"    ✗ {image.name} - Falló después de {max_retries} intentos")
                            errors += 1
                            success = True
                            img_idx += 1
                
                # Delay entre imágenes para no saturar el servidor
                time.sleep(1)
